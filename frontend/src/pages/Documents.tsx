...
const MOCK_DOCS: Document[] = [
  ...
  { id: 5, code: 'НПЗ-АР-003', title: 'Архитектурные решения', project_id: 1, status: 'approved', doc_type: 'АР', current_revision_id: 7 },
  { id: 6, code: 'ЛЭП-ПС-002', title: 'План силовой сети', project_id: 2, status: 'on_review', doc_type: 'ПС', current_revision_id: 8 },
  { id: 7, code: 'НПЗ-КМ-003', title: 'Фундаменты', project_id: 1, status: 'draft', doc_type: 'КМ', current_revision_id: null },
  { id: 8, code: 'КОТ-ОВ-002', title: 'Вентиляция', project_id: 3, status: 'in_progress', doc_type: 'ОВ', current_revision_id: 4 },
  { id: 9, code: 'ЛЭП-ЭС-003', title: 'Кабельный журнал', project_id: 2, status: 'approved', doc_type: 'ЭС', current_revision_id: 6 },
  { id: 10, code: 'НПЗ-ТХ-004', title: 'Технологическая схема', project_id: 1, status: 'on_review', doc_type: 'ТХ', current_revision_id: 9 },
];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-semibold text-[var(--text-primary)]">Документы</h1>
          <p className="text-sm text-[var(--text-secondary)] mt-1">Управление документами проекта</p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="relative">
            <Input
              ...
              className="col-span-full lg:col-span-1 pl-10"
            />
            <Search className="absolute left-3 top-9 w-4 h-4 text-[var(--text-tertiary)]" />
          </div>
          <Select
            label="Статус"
      ...
      <div className="table-container">
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-[var(--border-light)]">
            ...
            <tbody className="divide-y divide-[var(--border-light)]">
              {loading ? (
                Array.from({ length: Number(itemsPerPage) }).map((_, i) => (
                  <tr key={i}>
                    <td className="px-6 py-4 whitespace-nowrap"><div className="h-4 w-4 bg-[var(--bg-hover)] rounded" /></td>
                    <td className="px-6 py-4 whitespace-nowrap"><div className="h-4 bg-[var(--bg-hover)] rounded w-24" /></td>
                    <td className="px-6 py-4 whitespace-nowrap"><div className="h-4 bg-[var(--bg-hover)] rounded w-48" /></td>
                    <td className="px-6 py-4 whitespace-nowrap"><div className="h-4 bg-[var(--bg-hover)] rounded w-16" /></td>
                    <td className="px-6 py-4 whitespace-nowrap"><div className="h-4 bg-[var(--bg-hover)] rounded w-20" /></td>
                  </tr>
                ))
                ...
                paginatedDocs.map(d => (
                  <tr key={d.id} className="table-row cursor-pointer">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-[var(--text-primary)] group-hover:text-[var(--primary)]" onClick={() => navigate(`/documents/${d.id}`)}>{d.code}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-[var(--text-secondary)]" onClick={() => navigate(`/documents/${d.id}`)}>{d.title}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-[var(--text-tertiary)]" onClick={() => navigate(`/documents/${d.id}`)}>{d.doc_type || '—'}</td>
            ...
            <tfoot>
              <tr>
                <td colSpan={5} className="py-3 px-4 border-t border-[var(--border-default)] bg-[var(--bg-surface-2)]">
                  <nav className="flex items-center justify-between" aria-label="Пагинация документов">
                    <div className="flex items-center gap-2">
                      <button
                        onClick={() => setCurrentPage(prev => Math.max(1, prev - 1))}
                        disabled={currentPage === 1}
                        className="px-3 py-1.5 text-sm rounded-md border border-[var(--border-default)] bg-[var(--bg-surface)] text-[var(--text-primary)] hover:bg-[var(--bg-hover)] disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                        aria-label="Предыдущая страница"
                      >
                        Назад
                      </button>
                      <span 
                        className="px-4 py-1.5 text-sm font-medium text-[var(--text-primary)] min-w-[140px] text-center"
                        aria-current="page"
                      >
...
function TableHeader({ children, sortKey, currentSortBy, currentSortOrder, onSort }: TableHeaderProps) {
  ...
  return (
    <th 
      className="px-6 py-3 text-left text-xs font-semibold text-[var(--text-tertiary)] uppercase tracking-wider cursor-pointer select-none hover:text-[var(--text-secondary)] transition-colors"
      onClick={() => onSort(sortKey)}
    >
      <div className="flex items-center gap-1.5">
        {children}
        <span className="flex flex-col">
          <ChevronUp size={12} className={`${isAsc ? 'text-[var(--primary)]' : 'text-[var(--border-dark)]'}`} />
          <ChevronDown size={12} className={`${isDesc ? 'text-[var(--primary)]' : 'text-[var(--border-dark)]'} -mt-1`} />
        </span>
      </div>
    </th>
  );
